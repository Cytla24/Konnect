            ...
            if keyword in tweet.text:
                try:
                    ...
                    if tweet.id not in usedIds:
                        db.session.add(Tweet(id=tweet.id, name=tweet.user.screen_name, date_posted=tweet.created_at, content=tweet.text, business_id=biz_id))
                        db.session.commit()
                except:
                    print("error")
                b = 2
...
def addBusiness():
    ...
    business = Business(name=name, rating=rating, address=address, keywords=keyword)
